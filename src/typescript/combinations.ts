...
export class CombinationsUtil {
  ...
  private findCombination<T extends { value: number }>(current: T[], remaining: T[], remainingValue: number): T[][] {
    if (remainingValue === 0) {
      return [current];
    }
...
const combinationsUtil = new CombinationsUtil();
...
const targetValue = 2120.58; // Valor alvo

// console.log('Lista de objetos:', objects);
// console.log('Valor alvo:', targetValue);

console.time('exec');
const combinations = combinationsUtil.byValue(objects, targetValue);
console.log('Combinações encontradas:', combinations.length);
console.timeEnd('exec');
// console.log('Combinações encontradas:', combinations, null, 4);