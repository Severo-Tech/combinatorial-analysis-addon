...
def find_combinations(list_: List[ICombinationsValue], target: float, n):
    combinations_list = combinations(list_, n)
    return [combination for combination in combinations_list if sum(match['value'] for match in combination) == target]

class CombinationsUtil:
    def byValue(self, list_: List[ICombinationsValue], target: float) -> List[List[ICombinationsValue]]:
        inner_list = [{'id': el.get('id'), 'value': float(el.get('value'))} for el in list_]

        result = []

        combine = partial(find_combinations, inner_list, target)

        with Pool() as pool:
            num_combinations = range(1, len(inner_list) + 1)

            result = pool.map(combine, num_combinations)
...
if __name__ == '__main__':
    ...
    inicio = time.time()
    result = util.byValue(list_of_combinations, target_value)
    fim = time.time()
